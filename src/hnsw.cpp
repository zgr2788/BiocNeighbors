...
SEXP hnsw_builder(int nlinks, int ef_construct, int ef_search, std::string distance) {
    ...
    if (distance == "Manhattan") {
        return BiocNeighbors::BuilderPointer(
            new knncolle_hnsw::HnswBuilder<int, double, double>(knncolle_hnsw::makeManhattanDistanceConfig<float>(), opt)
        );

    } else if (distance == "Euclidean") {
        return BiocNeighbors::BuilderPointer(
            new knncolle_hnsw::HnswBuilder<int, double, double>(knncolle_hnsw::makeEuclideanDistanceConfig<float>(), opt)
        );

    } else if (distance == "Cosine") {
        return BiocNeighbors::BuilderPointer(
            new knncolle::L2NormalizedBuilder<int, double, double, double>(
                std::make_shared<knncolle_hnsw::HnswBuilder<int, double, double> >(knncolle_hnsw::makeEuclideanDistanceConfig<float>(), opt)
            )
        );
